...
generate_pdf_report            — write a PDF with chat output (thoughts + tool calls) and plots
generate_narrative_pdf_report  — write a PDF with agent explanations (thoughts) and plots only
...
def generate_pdf_report(
    ...
    """Generate a PDF report with session chat output and plot images.

    Renders the session conversation (agent thoughts and tool calls with
    their results) in a chat-UI style, followed by every PNG plot from the
    session's ``plots/`` directory (one per page). Writes
    ``report_<timestamp>.pdf`` to ``<session_dir>/reports/``.
    ...
    if chat_events:
        ...
        pdf.cell(W, 8, "  Chat Output", border="TB", fill=True,
                 new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_text_color(0, 0, 0)
        pdf.set_draw_color(0, 0, 0)
        pdf.ln(4)
        for ev in chat_events:
            if ev["type"] == "thought":
                _render_thought(pdf, ev, W)
            elif ev.get("tool") != "log_thought":
                _render_tool_call(pdf, ev, W)
    ...
    if not chat_events and n_plots == 0:
        pdf.add_page()
...
def generate_narrative_pdf_report(
    ...
    """Generate a PDF report with agent explanations (thoughts) and plot images.

    Like ``generate_pdf_report`` but renders *only* agent reasoning/thought
    blocks (logged via ``log_thought``) and the session plots.  Tool call
    cards are intentionally omitted so the output reads as a clean narrative
    explanation of what the agent did and found.

    Writes ``report_narrative_<timestamp>.pdf`` to ``<session_dir>/reports/``.
    ...
    if thoughts:
        ...
        pdf.set_font("Helvetica", "B", 11)
        pdf.set_text_color(30, 50, 120)
        pdf.cell(W, 8, "  Agent Reasoning", border="TB", fill=True,
                 new_x=XPos.LMARGIN, new_y=YPos.NEXT)
        pdf.set_text_color(0, 0, 0)
        pdf.set_draw_color(0, 0, 0)
        pdf.ln(4)
    ...
    reports_dir.mkdir(parents=True, exist_ok=True)
    ts_file  = _dt.now(_tz.utc).strftime("%Y%m%d_%H%M%S")
    pdf_path = reports_dir / f"report_narrative_{ts_file}.pdf"
    pdf.output(str(pdf_path))
